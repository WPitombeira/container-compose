...
1. `ContainerComposeCLI`
   ...
   - Owns user-facing commands: `config`, `plan`, `up`, `run`, `create`, `build`, `down`, `start`, `pull`, `push`, `images`, `stop`, `restart`, `kill`, `pause`, `unpause`, `attach`, `wait`, `scale`, `commit`, `rm`, `exec`, `cp`, `logs`, `ps`, `top`, and `stats`.
