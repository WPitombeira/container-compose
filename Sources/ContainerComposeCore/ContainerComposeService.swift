...
public struct ContainerComposePlanRequest: Equatable, Sendable {
    ...
    public var volumesOptions: AppleContainerVolumesOptions
    ...
    public init(
        ...
        self.volumesOptions = volumesOptions
...
public struct ContainerComposeService: Sendable {
    ...
    private func plannedCommands(
        ...
        case .unpause:
            return planner.planUnpause(project: project, services: request.services)
        case .attach:
            return planner.planAttach(
                project: project,
        ...
        case .scale:
            return planner.planScale(
                project: project,
                targets: request.scaleTargets,
                services: request.services,
                options: request.scaleOptions
            )
        case .commit:
            return planner.planCommit(
                project: project,
                service: request.services.first,
        ...
        case .volumes:
            return planner.planVolumes(
                project: project,
                services: request.services,
                options: request.volumesOptions
            )
        case .rm:
        ...
        case .top:
            return planner.planTop(project: project, services: request.services)
        case .stats:
