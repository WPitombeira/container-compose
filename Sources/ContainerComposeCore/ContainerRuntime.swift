...
public struct AppleContainerExecutionRunner {
    ...
    private func unsupportedPlanActionExecution(commandIndex: Int, command: PlannedCommand) -> PlannedCommandExecution? {
        guard command.action == .delegateService || command.action == .pauseService || command.action == .unpauseService || command.action == .attachService || command.action == .waitService || command.action == .scaleService || command.action == .commitService || command.action == .exportService || command.action == .eventsProject || command.action == .watchProject || command.action == .publishProject || command.action == .listProjects else { return nil }
