...
public struct AppleContainerExecutionRunner {
    ...
    private func unsupportedPlanActionExecution(commandIndex: Int, command: PlannedCommand) -> PlannedCommandExecution? {
        guard command.action == .delegateService || command.action == .pauseService || command.action == .unpauseService || command.action == .attachService || command.action == .waitService || command.action == .scaleService else { return nil }
