...
final class ContainerComposeServiceTests: XCTestCase {
    ...
    func testFacadePlansPauseForSelectedServicesWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        XCTAssertEqual(result.plan.operation, "pause")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.pauseService])
        XCTAssertEqual(result.plan.commands.first?.arguments, ["pause", "demo_web_1"])
        XCTAssertEqual(result.plan.commands.first?.diagnostics.first?.path, "pause")
    }

    func testFacadePlansUnpauseForSelectedServicesWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        XCTAssertEqual(result.plan.operation, "unpause")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.unpauseService])
        XCTAssertEqual(result.plan.commands.first?.arguments, ["unpause", "demo_web_1"])
        XCTAssertEqual(result.plan.commands.first?.diagnostics.first?.path, "unpause")
    }

    func testFacadePlansAttachForSelectedServiceWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        let result = try ContainerComposeService().makePlan(.init(
            ...
            projectName: "demo",
            services: ["web"],
            attachOptions: .init(attachStdin: false)
        ))

        XCTAssertEqual(result.plan.operation, "attach")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.attachService])
        XCTAssertEqual(result.plan.commands.first?.arguments, ["attach", "--no-stdin", "demo_web_1"])
    ...
    func testFacadePlansScaleForAssignmentsWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        let result = try ContainerComposeService().makePlan(.init(
            ...
            services: ["web"],
            scaleTargets: ["web": 2],
            scaleOptions: .init(noDependencies: true)
        ))

        XCTAssertEqual(result.plan.operation, "scale")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.scaleService])
        XCTAssertEqual(result.plan.commands.first?.arguments, ["scale", "--no-deps", "web=2"])
        XCTAssertEqual(result.plan.commands.first?.diagnostics.first?.path, "scale")
    }

    func testFacadePlansCommitForSelectedServiceWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        let result = try ContainerComposeService().makePlan(.init(
            ...
            projectName: "demo",
            services: ["web"],
            commitOptions: .init(message: "snapshot", repository: "example/web:snapshot")
        ))

        XCTAssertEqual(result.plan.operation, "commit")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.commitService])
        XCTAssertEqual(result.plan.commands.first?.arguments, [
            "commit",
            "--message", "snapshot",
            "demo_web_1",
            "example/web:snapshot"
        ])
        XCTAssertEqual(result.plan.commands.first?.diagnostics.first?.path, "commit")
    }

    func testFacadePlansExportForSelectedServiceWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        let result = try ContainerComposeService().makePlan(.init(
            ...
            projectName: "demo",
            services: ["web"],
            exportOptions: .init(output: "./web.tar")
        ))

        XCTAssertEqual(result.plan.operation, "export")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.exportService])
        XCTAssertEqual(result.plan.commands.first?.arguments, [
            "export",
            "--output", "./web.tar",
            "demo_web_1"
        ])
        XCTAssertEqual(result.plan.commands.first?.diagnostics.first?.path, "export")
    }

    func testFacadePlansEventsForSelectedServicesWithDiagnostic() throws {
        let workdir = try makeTemporaryWorkdir()
        defer { try? FileManager.default.removeItem(at: workdir) }
        ...
        let result = try ContainerComposeService().makePlan(.init(
            ...
            projectName: "demo",
            services: ["web"],
            eventsOptions: .init(outputJSON: true, since: "1h")
        ))

        XCTAssertEqual(result.plan.operation, "events")
        XCTAssertEqual(result.plan.selectedServices, ["web"])
        XCTAssertEqual(result.plan.commands.map(\.action), [.eventsProject])
        XCTAssertEqual(result.plan.commands.first?.arguments, ["events", "--json", "--since", "1h", "web"])
