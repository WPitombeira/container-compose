...
final class ContainerRuntimeTests: XCTestCase {
    ...
    func testExecutionRunnerFailsAttachCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .attachService, service: "web", arguments: ["attach", "demo_web_1"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action attachService is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsWaitCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .waitService, service: "web", arguments: ["wait", "demo_web_1"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action waitService is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsScaleCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .scaleService, service: "web", arguments: ["scale", "web=2"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action scaleService is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsCommitCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .commitService, service: "web", arguments: ["commit", "demo_web_1", "example/web:snapshot"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action commitService is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsExportCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .exportService, service: "web", arguments: ["export", "--output", "./web.tar", "demo_web_1"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action exportService is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }
    ...
    func testExecutionRunnerFailsWatchCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .watchProject, arguments: ["watch", "--no-up", "web"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action watchProject is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsPublishCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .publishProject, arguments: ["publish", "--app", "example/app:latest"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action publishProject is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsListProjectsCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .listProjects, arguments: ["compose", "ls", "--format", "json"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action listProjects is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }

    func testExecutionRunnerFailsListVolumesCommandsWithoutCallingExecutor() {
        let executor = FakeContainerCommandExecutor()
        let plan = makePlan(commands: [
            PlannedCommand(action: .listVolumes, arguments: ["volume", "list", "--format", "json"]),
            PlannedCommand(action: .startService, service: "worker", arguments: ["start", "demo_worker_1"])
        ])
        ...
        XCTAssertEqual(report.results.map(\.status), [.failed, .skipped])
        XCTAssertEqual(report.results[0].errorCode, .unsupportedPlanAction)
        XCTAssertEqual(report.results[0].error, "Planned action listVolumes is not executable through Apple Container yet.")
        XCTAssertEqual(report.results[1].errorCode, .skippedPreviousFailure)
    }
